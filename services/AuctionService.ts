import { Auction, Bid } from '../modules/Auction';
...
export class AuctionService {
    ...
    public placeBid(auctionId: string, bidderId: string, value: number): boolean {
        ...
        if (!auction || !bidder || auction.status !== AuctionStatus.ABERTO || value < auction.minimumBid) {
            return false;
        }

        const lastBid = auction.bids.length > 0 ? auction.bids[auction.bids.length - 1].value : auction.minimumBid;
        console.log(lastBid);
    ...
    public getLowestBidForAuction(auctionId: string): Bid | null {
        ...
        return auction.bids.reduce((prev, current) => (prev.value < current.value ? prev : current));
    }
